...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( rangeabs.length, 1, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the range of absolute values of an array', function test( t ) {
	var x;
	var v;
...
tape( 'the function calculates the range of absolute values of an array (accessors)', function test( t ) {
	var x;
	var v;

	x = [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ];
	v = rangeabs( toAccessorArray( x ) );
	t.strictEqual( v, 5.0, 'returns expected value' );
...
tape( 'the function calculates the range of absolute values of an array (array-like object)', function test( t ) {
	var x;
	var v;
...
tape( 'if provided an empty array, the function returns `NaN` (accessors)', function test( t ) {
	var v = rangeabs( toAccessorArray( [] ) );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
	t.end();
});
...
tape( 'if provided an array containing a single element, the function returns `0` (accessors)', function test( t ) {
	var v = rangeabs( toAccessorArray( [ 1.0 ] ) );
	t.strictEqual( v, 0.0, 'returns expected value' );
	t.end();
});
